...
class LaneDetection:
    ...
    def __init__(self, width, height, camera, lk):
        ...
        self.width = width
        self.lk = lk
        self.log = logging.getLogger('LaneDetection')
        ...
        # Stop line detection state
        self.stop_line_detected = False
        self.stop_line_consecutive_frames = 0
        self.stop_line_min_consecutive = int(self.config["LANE_DETECT"].get("stop_line_min_consecutive"))
        self.stop_line_max_allowed_slope = float(self.config["LANE_DETECT"].get("stop_line_max_allowed_slope"))
        self.print_stop_line = self.config["LANE_DETECT"].getboolean("print_stop_line")
        self.stop_line_segment = None
    ...
    def lanes_detection(self, src):
        ...
        lane_det_results = {
            ...
            "stop_line_segment": self.stop_line_segment,
            "stop_line_distance": self.stop_line_distance,
        }
    ...
    def visualize_horizontal_line(self, frame, detected_line_segment, line):
        if detected_line_segment:
            p1, p2 = detected_line_segment[0], detected_line_segment[1]
            # Outer glow
            cv2.line(frame, p1, p2, (0, 80, 200), thickness=10, lineType=cv2.LINE_AA)
            # Core line
            cv2.line(frame, p1, p2, (0, 140, 255), thickness=4, lineType=cv2.LINE_AA)
            # Bright center
            cv2.line(frame, p1, p2, (100, 200, 255), thickness=1, lineType=cv2.LINE_AA)
    ...
    def detect_stop_line(self, frame):
        """Detects a stop line using horizontal_detection() with temporal filtering.
        Updates internal state (stop_line_detected, stop_line_segment, stop_line_distance).
        Only confirms a stop line after it has been seen in multiple consecutive frames.

        Parameters
        ----------
        frame : array
            Input image (will be drawn on if print_stop_line is ON)

        Returns
        -------
        bool
            Whether a confirmed stop line is currently detected
        """
        hor_frame = frame.copy()
        detected_line_segment, line, hor_exists = self.horizontal_detection(hor_frame, self.stop_line_max_allowed_slope)

        if hor_exists:
            self.stop_line_consecutive_frames += 1
            self.stop_line_segment = detected_line_segment
            y_avg = (detected_line_segment[0][1] + detected_line_segment[1][1]) / 2
            self.stop_line_distance = y_avg / self.height
        else:
            self.stop_line_consecutive_frames = 0
            self.stop_line_segment = None
            self.stop_line_distance = None

        self.stop_line_detected = self.stop_line_consecutive_frames >= self.stop_line_min_consecutive

        if self.stop_line_detected and self.print_stop_line:
            self.visualize_horizontal_line(frame, self.stop_line_segment, line)
            # Warning panel
            panel_w, panel_h = 200, 40
            px, py = self.width - panel_w - 10, 10
            overlay = frame.copy()
            cv2.rectangle(overlay, (px, py), (px + panel_w, py + panel_h), (0, 0, 60), -1)
            cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
            cv2.rectangle(frame, (px, py), (px + panel_w, py + panel_h), (0, 0, 200), 2)
            cv2.putText(frame, "STOP LINE", (px + 8, py + 16),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.45, (0, 100, 255), 1, cv2.LINE_AA)
            # Distance bar
            bar_x, bar_y = px + 8, py + 24
            bar_w = panel_w - 16
            dist_fill = int(bar_w * self.stop_line_distance)
            cv2.rectangle(frame, (bar_x, bar_y), (bar_x + bar_w, bar_y + 8), (40, 40, 40), -1)
            cv2.rectangle(frame, (bar_x, bar_y), (bar_x + dist_fill, bar_y + 8), (0, 140, 255), -1)
            cv2.rectangle(frame, (bar_x, bar_y), (bar_x + bar_w, bar_y + 8), (80, 80, 80), 1)
            cv2.putText(frame, f"{self.stop_line_distance:.0%}", (bar_x + bar_w + 4, bar_y + 8),
    ...
    def create_lanes_from_peaks(self, frame, left, right):
        ...
        if self.print_lanes:
            self.visualize_lane_zone(left_coef, right_coef, frame)
            self.visualize_lane(left_coef, frame, bgr_colour=(255, 165, 0))
            self.visualize_lane(right_coef, frame, bgr_colour=(0, 165, 255))
    ...
    def visualize_lane(self, coefs, frame, bgr_colour=(102, 0, 102)):
        """Visualizes a lane with a glowing neon effect.
        ...
        end_h = int((1 - self.bottom_perc) * self.height)
        points = []
        for i in range(self.bottom_row_index, end_h, self.step):
            x = int(a * i**2 + b * i + c)
            if 0 <= x <= self.width:
                points.append((x, i))
        ...
        for i in range(1, len(points)):
            cv2.line(frame, points[i - 1], points[i], bgr_colour, thickness=8)
        # Bright core
        bright = tuple(min(255, int(ch * 1.5)) for ch in bgr_colour)
        for i in range(1, len(points)):
            cv2.line(frame, points[i - 1], points[i], bright, thickness=2)

        return

    def visualize_lane_zone(self, left_coefs, right_coefs, frame, bgr_colour=(0, 200, 100), alpha=0.15):
        """Draws a semi-transparent filled polygon between the two detected lanes.
        ...
        al, bl, cl = left_coefs[0], left_coefs[1], left_coefs[2]
        ar, br, cr = right_coefs[0], right_coefs[1], right_coefs[2]

        end_h = int((1 - self.bottom_perc) * self.height)
        left_pts = []
        right_pts = []
        for i in range(self.bottom_row_index, end_h, self.step):
            xl = int(al * i**2 + bl * i + cl)
            xr = int(ar * i**2 + br * i + cr)
            if 0 <= xl <= self.width and 0 <= xr <= self.width:
                left_pts.append([xl, i])
                right_pts.append([xr, i])
        ...
        overlay = frame.copy()
        cv2.fillPoly(overlay, [polygon], bgr_colour)
    ...
    def visualize_all_peaks(self, frame, peaks, bgr_colour=(180, 180, 180)):
        """Draws small diamond markers for all detected peaks (unassigned).
        ...
        for peak in peaks:
            ...
            cv2.polylines(frame, [pts], True, bgr_colour, 1, cv2.LINE_AA)

        return

    def visualize_peaks(self, frame, left, right):
        """Draws assigned lane points with filled circles and outer rings.
        ...
        for peak in left:
            point = (peak[0], peak[1])
            cv2.circle(frame, point, 4, (255, 165, 0), -1, cv2.LINE_AA)
            cv2.circle(frame, point, 4, (255, 220, 150), 1, cv2.LINE_AA)

        for peak in right:
            point = (peak[0], peak[1])
            cv2.circle(frame, point, 4, (0, 165, 255), -1, cv2.LINE_AA)
            cv2.circle(frame, point, 4, (150, 220, 255), 1, cv2.LINE_AA)

        return

    def visualize_lane_certainty(self, frame, l_perc, r_perc):
        """Draws HUD-style certainty bars for left and right lanes.
        ...
        frame : array
            Input image
        l_perc : float
            certainty of the left lane (0-100)
        r_perc : float
            certainty of the right lane (0-100)
        """
        bar_w = 80
        bar_h = 8
        margin = 10

        for label, perc, color, x_base in [
            ("L", l_perc, (255, 165, 0), margin),
            ("R", r_perc, (0, 165, 255), self.width - bar_w - margin - 25),
        ]:
            perc_clamped = max(0, min(100, perc))
            fill_w = int(bar_w * perc_clamped / 100)
            y = self.height - margin - bar_h

            # Label
            cv2.putText(frame, label, (x_base, y + bar_h - 1),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.35, (220, 220, 220), 1, cv2.LINE_AA)

            bx = x_base + 15
            # Background bar
            overlay = frame.copy()
            cv2.rectangle(overlay, (bx, y), (bx + bar_w, y + bar_h), (40, 40, 40), -1)
            cv2.addWeighted(overlay, 0.6, frame, 0.4, 0, frame)
            # Fill bar — color shifts from red (low) to green (high)
            g = int(perc_clamped * 2.55)
            r_ch = int((100 - perc_clamped) * 2.55)
            fill_color = (0, g, r_ch)
            cv2.rectangle(frame, (bx, y), (bx + fill_w, y + bar_h), fill_color, -1)
            # Border
            cv2.rectangle(frame, (bx, y), (bx + bar_w, y + bar_h), (100, 100, 100), 1)
            # Percentage text
            cv2.putText(frame, f"{perc_clamped:.0f}%", (bx + bar_w + 4, y + bar_h - 1),
    ...
    def _draw_badge(self, frame, text, center_x, center_y, bg_color, text_color=(240, 240, 240)):
        """Draws a rounded pill-shaped badge with text."""
        (tw, th), _ = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, 0.4, 1)
        pad_x, pad_y = 10, 6
        x1 = center_x - tw // 2 - pad_x
        y1 = center_y - th // 2 - pad_y
        x2 = center_x + tw // 2 + pad_x
        y2 = center_y + th // 2 + pad_y
        overlay = frame.copy()
        cv2.rectangle(overlay, (x1, y1), (x2, y2), bg_color, -1, cv2.LINE_AA)
        cv2.addWeighted(overlay, 0.7, frame, 0.3, 0, frame)
        cv2.rectangle(frame, (x1, y1), (x2, y2), bg_color, 1, cv2.LINE_AA)
        cv2.putText(frame, text, (center_x - tw // 2, center_y + th // 2),
    ...
    def check_for_dashed(self, left, right, frame=None):
        ...
        if left:
            ...
            if dashed_l and self.print_if_dashed and frame is not None:
                self._draw_badge(frame, "DASHED", int(0.2 * self.width),
                                 int(self.height * 0.28), (180, 120, 0))

        if right:
            dashed_r = self.is_dashed(right)
            if dashed_r and self.print_if_dashed and frame is not None:
                self._draw_badge(frame, "DASHED", int(0.8 * self.width),
                                 int(self.height * 0.28), (0, 120, 180))
    ...
    def choose_405(self):
        ...
        # FIX: keep indices in [0, height-1]
